...
function VmCallNode(callDetails)
{
    ...
    this.resultType = callDetails.returnType;
}
...
function ProcedureDefinition(name, parameters, statements)
{
	...
	this.variables  = {};
	this.returnType = [Types.void]; // This will be determined later by looking for "output"
}
...
ProcedureDefinition.prototype.compile =
	function ()
	{
		...
		else
		{
			...
			this.nodes.returnType = eType.void;
		}
		...
		//this.nodes.returnType = eType.UNKNOWN; // for now until I figure this all out
	};
...
*/		
		...
		var nodeBlock = new VmOperationNode(block.returnType, codeBlock);
		var nodeEob   = new VmOperationNode([Types.void], new VmCode("eob"));

		return new VmOperationNode(block.returnType, null, nodeBlock, block, nodeEob);
	};
...
BabuinoBackEnd.prototype.compileOutput =
	function (arg)
	{
		return new VmOperationNode(arg.returnType, new VmCode("output"), arg);
	};

BabuinoBackEnd.prototype.compileSimpleCommand =
	function (cmd, returnType)
	{
		return new VmOperationNode(returnType, new VmCode(cmd));
	};

BabuinoBackEnd.prototype.compileArgCommand =
	function (cmd, arg, returnType)
	{
		return new VmOperationNode(returnType, new VmCode(cmd), arg);
	};
...
BabuinoBackEnd.prototype.compileNumericExpression =
	function (lhs, op, rhs)
	{
		...
		return new VmOperationNode([returnType], newOp, lhs, rhs);
	};
...
BabuinoBackEnd.prototype.compileBooleanExpression =
	function (lhs, op, rhs)
	{
		...
		{
			var conversion = getConversionCode(rhs.returnType[0], lhs.returnType[0]);
			rhs.append(conversion);
            returnType = lhs.returnType[0];
		}
		else if(lhs.returnType[0].size < rhs.returnType[0].size)	// Comparing two numbers
		{
			var conversion = getConversionCode(lhs.returnType[0], rhs.returnType[0]);
			lhs.append(conversion);
