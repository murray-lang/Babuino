...
/**
 * @fileoverview Generating Logo for unit test blocks.
...
Blockly.Logo.unittest_main = function() {
  // Container for unit tests.
  var resultsVar = Blockly.Logo.variableDB_.getName('unittestResults',
      Blockly.Variables.NAME_TYPE);
  if (!Blockly.Logo.definitions_['unittest_report']) {
    var functionName = Blockly.Logo.variableDB_.getDistinctName(
        'testReport', Blockly.Generator.NAME_TYPE);
    Blockly.Logo.unittest_main.report = functionName;
    var func = [];
    ...
    func.push('}');
    func.push('');
    Blockly.Logo.definitions_['unittest_report'] = func.join('\n');
  }
  // Setup global to hold test results.
  var code = resultsVar + ' = [];\n';
  // Run tests (unindented).
  code += Blockly.Logo.statementToCode(this, 'DO')
      .replace(/^  /, '').replace(/\n  /g, '\n');
  var reportVar = Blockly.Logo.variableDB_.getDistinctName(
      'report', Blockly.Variables.NAME_TYPE);
  code += 'var ' + reportVar + ' = ' +
      Blockly.Logo.unittest_main.report + '();\n';
  // Destroy results.
...
Blockly.Logo.unittest_main.defineAssert_ = function() {
  if (!Blockly.Logo.definitions_['unittest_assertequals']) {
    var resultsVar = Blockly.Logo.variableDB_.getName('unittestResults',
        Blockly.Variables.NAME_TYPE);
    var functionName = Blockly.Logo.variableDB_.getDistinctName(
        'assertEquals', Blockly.Generator.NAME_TYPE);
    Blockly.Logo.unittest_main.assert_ = functionName;
    var func = [];
    ...
    func.push('}');
    func.push('');
    Blockly.Logo.definitions_['unittest_assertequals'] = func.join('\n');
  }
  return Blockly.Logo.unittest_main.assert_;
};

Blockly.Logo.unittest_assertequals = function() {
  // Asserts that a value equals another value.
  var resultsVar = Blockly.Logo.variableDB_.getName('unittestResults',
      Blockly.Variables.NAME_TYPE);
  var message = Blockly.Logo.quote_(this.getTitleValue('MESSAGE'));
  var actual = Blockly.Logo.valueToCode(this, 'ACTUAL',
      Blockly.Logo.ORDER_COMMA) || 'null';
  var expected = Blockly.Logo.valueToCode(this, 'EXPECTED',
      Blockly.Logo.ORDER_COMMA) || 'null';
  return Blockly.Logo.unittest_main.defineAssert_() +
      '(' + actual + ', ' + expected + ', ' + message + ');\n';
};

Blockly.Logo.unittest_assertvalue = function() {
...
Blockly.Logo.unittest_fail = function() {
  // Always assert an error.
  var resultsVar = Blockly.Logo.variableDB_.getName('unittestResults',
      Blockly.Variables.NAME_TYPE);
  var message = Blockly.Logo.quote_(this.getTitleValue('MESSAGE'));
  if (!Blockly.Logo.definitions_['unittest_fail']) {
    var functionName = Blockly.Logo.variableDB_.getDistinctName(
        'fail', Blockly.Generator.NAME_TYPE);
    Blockly.Logo.unittest_fail.assert = functionName;
    var func = [];
    ...
    func.push('}');
    func.push('');
    Blockly.Logo.definitions_['unittest_fail'] = func.join('\n');
  }
  return Blockly.Logo.unittest_fail.assert + '(' + message + ');\n';
};
