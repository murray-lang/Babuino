 ...
 * @this {!Blockly.FieldVariable}
 */
Blockly.FieldVariable.dropdownChange = function(text, callback) {
  /*
  ...
  */
  ...
  function promptName(title, promptText, defaultText) {
    Blockly.hideChaff();
	
	var promptHandler = function(response) {
      var newVar = response && response.replace(/[\s\xa0]+/g, ' ').replace(/^ | $/g, '');
	  if (newVar)
	  {
		if (title == Blockly.MSG_RENAME_VARIABLE)
		{
			Blockly.Variables.renameVariable(defaultText, newVar);
		}
	  } else if (text == Blockly.MSG_NEW_VARIABLE)
	  {
			Blockly.Variables.renameVariable(newVar, newVar);
	  }
	  if (callback)
		    callback(text, newVar);
	}
    ...
    prompt.setDefaultValue(defaultText);
  ...
  if (text == Blockly.MSG_RENAME_VARIABLE) {
    ...
    promptName(text, Blockly.MSG_RENAME_VARIABLE_TITLE.replace('%1', oldVar),
                      oldVar);
    /*
	if (text) {
	...
	*/
    ...
    promptName(text, Blockly.MSG_NEW_VARIABLE_TITLE, '');
