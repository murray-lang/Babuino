...
Babuino::withInt16()
{
	...
	case OP_SET:
		...
		popUint16(_stack, (uint16_t*)&rhs);
		//Serial.print(location);
		//Serial.print("] = ");
		//Serial.println(rhs);
		setUint16(_stack, location, (uint16_t)rhs);
		return true;

	case OP_GET:
		//Serial.print("get [");
		popStackPtr(_stack, &location);
		getUint16(_stack, location, (uint16_t*)&rhs);
		//Serial.print(location);
		//Serial.print("] == ");
		//Serial.println(rhs);
	...
	case OP_ASET:
		...
		popUint16(_stack, (uint16_t*)&rhs);
		setUint16(_stack, location + index * sizeof(int16_t), (uint16_t)rhs);
		return true;
	...
	case OP_AGET:
		...
		popStackPtr(_stack, &location);
		popStackPtr(_stack, &index);
		getUint16(_stack, location + index * sizeof(int16_t), (uint16_t*)&rhs);
		pushUint16(_stack, (uint16_t)rhs);
		return true;
	...
	case OP_OUTPUT:
		{
			//Serial.print("output ");
			popUint16(_stack, (uint16_t*)&rhs);	// The return value
			...
			setUint16(_stack, location - (STACKPTR)argsSize - sizeof(int16_t), (uint16_t)rhs);
		}
		return true;
	...
	case OP_SEND:
		{
			...
			hton(rhs, buf);
			//_defaultStream->write(nbuf, sizeof(int16_t)); // Replaced for debugging
	...
	case OP_SENDN:
		{
			...
			int16_t* buf = (int16_t*)getStackAddress(_stack, bufLoc);
	...
	case OP_FOREACH:
		{	
			...
			if (itemsRemaining > 0)				// Any items remaining
			{
				...
				setStackPtr(_stack, location, getTop(_stack) - sizeof(int16_t));
