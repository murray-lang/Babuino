...
Babuino::withUint32()
{
	...
	case OP_SET:
		...
		popUint32(_stack, &rhs);

		setUint32(_stack, location, rhs);
		return true;

	case OP_GET:
		//Serial.println("---get---");
		popStackPtr(_stack, &location);
		getUint32(_stack, location, &rhs);
		pushUint32(_stack, rhs);
		return true;
	...
	case OP_ASET:
		...
		popUint32(_stack, &rhs);
		setUint32(_stack, location + index * sizeof(uint32_t), rhs);
		return true;
	...
	case OP_AGET:
		...
		popStackPtr(_stack, &location);
		popStackPtr(_stack, &index);
		getUint32(_stack, location + index * sizeof(uint32_t), &rhs);
		pushUint32(_stack, rhs);
		return true;
	...
	case OP_OUTPUT:
		{
			//Serial.print("output ");
			popUint32(_stack, &rhs);	// The return value
			...
			setUint32(_stack, location - (STACKPTR)argsSize - sizeof(uint32_t), rhs);
		}
		return true;
	...
	case OP_SEND:
		{
			...
			hton(rhs, buf);
			//_defaultStream->write(nbuf, sizeof(uint32_t)); // Replaced for debugging
	...
	case OP_SENDN:
		{
			...
			uint32_t* buf = (uint32_t*)getStackAddress(_stack, bufLoc);
	...
	case OP_FOREACH:
		{	
			...
			if (itemsRemaining > 0)				// Any items remaining
			{
				...
				setStackPtr(_stack, location, getTop(_stack) - sizeof(uint32_t));
